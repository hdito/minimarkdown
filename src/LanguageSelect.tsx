...
export const LanguageSelect = () => {
  ...
  return (
    <div className="group relative max-h-[30px]">
      ...
      <div className="absolute w-[110px] divide-y-2 divide-black dark:divide-gray-100 z-10 hidden group-focus-within:flex border-2 border-black dark:border-gray-100 rounded-md bg-white dark:bg-gray-800 group-focus-within:flex-col overflow-hidden">
        {Object.keys(languages).map((languageCode) => (
          <button
            onClick={() => i18n.changeLanguage(languageCode)}
            key={languageCode}
            className={`${
              languageCode === i18n.language ? 'font-bold' : ''
            } py-0.5 px-2`}
          >
